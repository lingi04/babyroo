...
- 사용자 맥락 카드
  - 선택된 아이들의 월령과 기본 지역
  - `{아이이름들} 기준으로 탐색`
  - `설정`을 누르면 설정 화면으로 이동한다.
- 검색 필드 placeholder: `행사 제목 검색`
- 필터 칩:
  - 활성화된 필터 요약
  - `필터` 또는 `필터 {활성필터수}`
- 결과 개수 텍스트:
...
검색:
- 검색어는 행사 제목에 대해서만 부분 일치로 적용한다.
- 지역, 동네, 가격, 월령, 실내/실외, 예약 여부는 검색어가 아니라 필터로 찾는다.
...
- 추천 준비 카드
  - 라벨: `추천 준비`
  - 제목: `조건에 맞는 후보만 추려볼까요?`
  - 설명: `선택한 조건을 기준으로 아이에게 맞는 후보를 먼저 보여드립니다.`
  - 기준 칩:
    - 선택된 아이들의 월령. 누르면 설정 화면으로 이동한다.
    - 기본 지역. 누르면 설정 화면으로 이동한다.
  - CTA:
    - 추천 전: `추천 받기 · 1회 사용`
    - 추천 후: `다시 추천 받기 · 1회 사용`
  - 추천 질문 목록은 준비 카드에 미리 표시하지 않는다.
  - 사용자가 질문에 답하기 시작하면 `선택한 답변` 요약으로 이미 고른 값을 보여준다.

추천 질문:
- 추천 탭은 탐색 필터를 그대로 재사용하지 않는다.
- `추천 받기`를 누르면 질문 카드가 열리고, 한 번에 하나의 질문만 보여준다.
- 질문 문구와 선택지는 모두 하드코딩한다.
- 핵심 질문은 항상 묻고, 적응형 질문은 하드코딩된 풀에서 상황에 맞는 하나를 선택한다.
- 핵심 질문:
  - `오늘 어디에서 출발하세요?`
  - `언제쯤 갈 생각인가요?`
  - `가는 날 날씨는 어떤가요?`
  ...
  - `예약이 필요한 행사도 괜찮으세요?`
- 적응형 질문:
  - 선택된 아이 중 24개월 미만 아이가 있으면 짧은 코스 선호를 묻는다.
  - 그렇지 않고 후보 데이터에 실내/야외가 모두 있으면 실내/야외 선호를 묻는다.
  - 그렇지 않으면 체험/전시 선호를 묻는다.
- 마지막 질문에 답하면 새 `RecommendationSession`을 생성한다.

추천 전 상태:
- 섹션 제목: `추천 전 확인`
  - 설명: `추천 받기를 누르면 선택한 조건으로 후보가 표시됩니다`
- 빈 상태 카드:
  - `추천 결과를 아직 만들지 않았어요`
...
현재 추천 결과 생성 방식:
- 현재는 LLM 호출이 아니라 로컬 규칙 기반 ranking이다.
- 질문 답변, 선택된 아이 월령, 출발 지역, 방문 날짜, 날씨, 가격, 예약 상태, 실내/야외, 카테고리를 사용해 후보를 압축한다.
