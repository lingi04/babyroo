...
test('polls a running recommendation session until it has results', async () => {
  const fetchMock = jest
    .spyOn(globalThis, 'fetch')
    .mockImplementation(async input => {
      const url = String(input);

      if (url.endsWith('/recommendation-sessions')) {
        return {
          ok: true,
          json: async () => ({
            id: 'rec-running',
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    expect.stringContaining('/recommendation-sessions/rec-running'),
    expect.objectContaining({
      method: 'GET',
    }),
  );
